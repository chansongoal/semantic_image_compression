...
def train(args):
  ...
  data_dict = vgg16.loadWeightsData('/gdata/gaocs/pretrained_models/vgg16_no_fc.npy')
  # Create input data pipeline.
  with tf.device("/cpu:0"):
    train_files = glob.glob(args.train_glob)
    train_files = preprocess_image_files(train_files)
    if not train_files:
      raise RuntimeError(
  ...
  # extract vgg16 feature for x and x_tilde
  # print(type(x), x.shape)   #Tensor, (?,256,256,3)
  ...
  vgg_x_tilde = custom_vgg16.custom_Vgg16(x_tilde, data_dict=data_dict)
  feature_x_tilde = [vgg_x_tilde.conv1_2, vgg_x_tilde.conv2_2, vgg_x_tilde.conv3_3, vgg_x_tilde.conv4_3, vgg_x_tilde.conv5_3]

  loss_f = tf.zeros([1,args.batchsize], tf.float32)
  for f, f_ in zip(feature_x, feature_x_tilde):
    # print(f.shape)
  ...
  # train_loss = args.lmbda * loss_f + train_bpp
  ...
  with tf.train.MonitoredTrainingSession(
      ...
      save_checkpoint_secs=300, save_summaries_secs=60, \
...
def decompress(args):
  ...
  # with open('COCO_val2014_000000000042.bin', "rb") as f:
  ...
  # op = write_png('COCO_val2014_000000000042.png', x_hat)
